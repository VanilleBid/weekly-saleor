...
def test_homepage_block_test_permissions(staff_client, authorized_client):
    ...
    for url in urls.urlpatterns:
        for client in clients:
            kwargs = {}
            pattern = url.pattern if hasattr(url, 'pattern') else url.regex.pattern
...
def test_homepage_block_list(admin_user: User):
    ...
    for current_pos, current_block in enumerate(zip(blocks, received_blocks)):
        ...
        assert current_recv_block.position == current_block.position == current_pos
        assert current_recv_block.pk == current_block.pk


def test_homepage_block_delete(admin_client: Client, homepage_block: HomePageItem):
    assert HomePageItem.objects.count() == 1
    endpoint = reverse(BASE_ENDPOINT + 'delete', kwargs={'pk': homepage_block.pk})
...
def test_homepage_block_add__bad_form(admin_client: Client, default_category, product_image):
    assert HomePageItem.objects.count() == 0
...
def test_homepage_block_add__missing_target(admin_client: Client, product_image):
    assert HomePageItem.objects.count() == 0
...
def test_homepage_block_add__too_many_targets(
        admin_client: Client, default_category, collection, page, product_image):
    assert HomePageItem.objects.count() == 0
...
def test_homepage_block_add(admin_client: Client, default_category, product_image):
    assert HomePageItem.objects.count() == 0
...
def test_homepage_block_edit(
    ...
    with mock.patch.object(SizedImage, 'create_resized_image'):
        def _test(data: dict, expected_target: Model, expected_warmup: bool, expected_status):
            with mock.patch.object(Warmer, '__call__') as mocked:
                resp = admin_client.post(
                    data=data,
                    path=reverse(BASE_ENDPOINT + 'edit', kwargs={'pk': homepage_block.pk}))
                ...
                if expected_status == 302:
                    ...
                    assert block.linked_object
                    assert isinstance(block.linked_object, type(expected_target))
